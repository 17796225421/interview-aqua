...
2017-08-06 [MySQL](https://draveness.me/tags/mysql) [InnoDB](https://draveness.me/tags/innodb) [数据库](https://draveness.me/tags/数据库)
...
![Database - Instance](image/Database-Instance.jpg)
...
![Logical-View-of-MySQL-Architecture](image/Logical-View-of-MySQL-Architecture.jpg)
...
![Tablespace-segment-extent-page-row](image/Tablespace-segment-extent-page-row.jpg)

同一个数据库实例的所有表空间都有相同的页大小；默认情况下，表空间中的页大小都为 16KB，当然也可以通过改变 `innodb_page_size` 选项对默认大小进行修改，需要注意的是不同的页大小最终也会导致区大小的不同：

![Relation Between Page Size - Extent Size](image/Relation-Between-Page-Size-Extent-Size.png)
...
![frm-and-ibd-file](image/frm-and-ibd-file.jpg)
...
![frm-file-hex](image/frm-file-hex.png)
...
![Antelope-Barracuda-Row-Format](image/Antelope-Barracuda-Row-Format.jpg)

> Antelope 是 InnoDB 最开始支持的文件格式，它包含两种行格式 Compact 和 Redundant，它最开始并没有名字；Antelope 的名字是在新的文件格式 Barracuda 出现后才起的，Barracuda 的出现引入了两种新的行格式 Compressed 和 Dynamic；InnoDB 对于文件格式都会向前兼容，而官方文档中也对之后会出现的新文件格式预先定义好了名字：Cheetah、Dragon、Elk 等等。

两种行记录格式 Compact 和 Redundant 在磁盘上按照以下方式存储：

![COMPACT-And-REDUNDANT-Row-Format](image/COMPACT-And-REDUNDANT-Row-Format.jpg)
...
![Row-Overflo](image/Row-Overflow.jpg)

但是当我们使用新的行记录格式 Compressed 或者 Dynamic 时都只会在行记录中保存 20 个字节的指针，实际的数据都会存放在溢出页面中。

![Row-Overflow-in-Barracuda](image/Row-Overflow-in-Barracuda.jpg)
...
![InnoDB-B-Tree-Node](image/InnoDB-B-Tree-Node.jpg)
...
![Infimum-Rows-Supremum](image/Infimum-Rows-Supremum.jpg)
...
![Clustered-Index](image/Clustered-Index.jpg)
...
![Optimistic-Pessimistic-Locks](image/Optimistic-Pessimistic-Locks.jpg)
...
![Shared-Exclusive-Lock](image/Shared-Exclusive-Lock.jpg)
...
![Lock-Type-Compatibility-Matrix](image/Lock-Type-Compatibility-Matrix.jpg)
...
![Transaction-Isolation-Matrix](image/Transaction-Isolation-Matrix.jpg)
...
![Read-Uncommited-Dirty-Read](image/Read-Uncommited-Dirty-Read.jpg)
...
![Read-Commited-Non-Repeatable-Read](image/2019-07-16-Read-Commited-Non-Repeatable-Read.jpg)
...
![Repeatable-Read-Phantom-Read](image/Repeatable-Read-Phantom-Read.jpg)

这种现象在数据库中就被称作幻读，虽然我们使用查询语句得到了一个空的集合，但是插入数据时却得到了错误，好像之前的查询是幻觉一样。

在标准的事务隔离级别中，幻读是由更高的隔离级别 `SERIALIZABLE` 解决的，但是它也可以通过 MySQL 提供的 Next-Key 锁解决：

![Repeatable-with-Next-Key-Lock](image/Repeatable-with-Next-Key-Lock.jpg)
