说起MySQL的查询优化，相信大家积累一堆技巧：不能使用`SELECT *`、不使用NULL字段、合理创建索引、为字段选择合适的数据类型….. 你是否真的理解这些优化技巧？是否理解其背后的工作原理？在实际场景下性能真有提升吗？
...
![图片](image/640-1639225129664777.webp)MySQL逻辑架构，来自：高性能MySQL
...
![图片](image/640-1639225129664778.webp)MySQL查询过程
...
![图片](image/640-1639225129665779.webp)二叉查找树和平衡二叉树
...
![图片](image/640-1639225129665780.webp)平衡二叉树旋转
...
![图片](image/640-1639225129665781.webp)简化B+Tree
...
![图片](image/640-1639225129665782.webp)leaf page和index page都没有满

接着插入下一个节点70，在`Index Page`中查询后得知应该插入到`50 - 70`之间的叶子节点，但叶子节点已满，这时候就需要进行也分裂的操作，当前的叶子节点起点为50，所以根据中间值来拆分叶子节点，如下图所示。

![图片](image/640-1639225129665783.webp)Leaf Page拆分

最后插入一个节点95，这时候`Index Page`和`Leaf Page`都满了，就需要做两次拆分，如下图所示。

![图片](image/640-1639225129665784.webp)Leaf Page与Index Page拆分

拆分后最终形成了这样一颗树。

![图片](image/640-1639225129665785.webp)最终树
...
![图片](image/640-1639225129665786.webp)左旋操作
...
![图片](image/640-1639225129665787.webp)索引如何组织数据存储，来自：高性能MySQL
