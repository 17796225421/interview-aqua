...
![图片](image/640-163918873097420.webp)

那么当 tcpdump 抓取到 icmp 数据包后， 输出格式如下：

![图片](image/640-163918873097421.webp)

![图片](image/640-163918873097422.webp)
...
![图片](image/640-163918873097423.webp)tcpdump 常用选项类
...
![图片](image/640-163918873097424.webp)tcpdump 常用过滤表达式类
...
![图片](image/640-163918873097425.webp)

接着把 ping.pcap 文件拖到电脑，再用 Wireshark 打开它。打开后，你就可以看到下面这个界面：

![图片](image/640-163918873097426.webp)
...
![图片](image/640-163918873097427.webp)ping 网络包
...
![图片](image/640-163918873097428.webp)
...
![图片](image/640-163918873097532.webp)TCP 流量图
...
![图片](image/640-163918873097533.webp)取消序列号相对值显示

取消后，Seq 显示的就是真实值了：

![图片](image/640-163918873097534.webp)TCP 流量图

可见，客户端和服务端的序列号实际上是不同的，序列号是一个随机值。

这其实跟我们书上看到的 TCP 三次握手和四次挥手很类似，作为对比，你通常看到的 TCP 三次握手和四次挥手的流程，基本是这样的：

![图片](image/640-163918873097535.webp)TCP 三次握手和四次挥手的流程
...
![图片](image/640-163918873097536.webp)四次挥手
...
![图片](image/640-163918873097537.webp)实验环境
...
![图片](image/640-163918873097538.webp)

其间 tcpdump 抓包的命令如下：

![图片](image/640-163918873097639.webp)

过了一会， curl 返回了超时连接的错误：

![图片](image/640-163918873097640.webp)

从 `date` 返回的时间，可以发现在超时接近 1 分钟的时间后，curl 返回了错误。

接着，把 tcp_sys_timeout.pcap 文件用 Wireshark 打开分析，显示如下图：

![图片](image/640-163918873097641.webp)SYN 超时重传五次
...
![图片](image/640-163918873097642.webp)SYN 超时重传两次
...
![图片](image/640-163918873097647.webp)
...
![图片](image/640-163918873097648.webp)
...
![图片](image/640-163918873097649.webp)

接着，在客户端执行如下 tcpdump 命令：

![图片](image/640-163918873097750.webp)

然后，客户端向服务端发起 telnet，因为 telnet 命令是会发起 TCP 连接，所以用此命令做测试：

![图片](image/640-163918873097751.webp)

此时，由于服务端收不到第三次握手的 ACK 包，所以一直处于 `SYN_RECV` 状态：

![图片](image/640-163918873097752.webp)

而客户端是已完成 TCP 连接建立，处于 `ESTABLISHED` 状态：

![图片](image/640-163918873097753.webp)

过了 1 分钟后，观察发现服务端的 TCP 连接不见了：

![图片](image/640-163918873097754.webp)
...
![图片](image/640-163918873097755.webp)

持续「好长」一段时间，客户端的 telnet 才断开连接：

![图片](image/640-163918873097756.png)
...
![图片](image/640-163918873097757.webp)
...
![图片](image/640-163918873097758.webp)
...
![图片](image/640-163918873097759.webp)常规 HTTP 请求

在 Linux 3.7 内核版本中，提供了 TCP Fast Open 功能，这个功能可以减少 TCP 连接建立的时延。

![图片](image/640-163918873097760.webp)常规 HTTP 请求 与 Fast  Open HTTP 请求
...
![图片](image/640-163918873097761.webp)TCP Fast Open 抓包分析
...
![图片](image/640-163918873097762.webp)快速重传机制

TCP 重复确认和快速重传的一个案例，用 Wireshark 分析，显示如下：

![图片](image/640-163918873097763.webp)
...
![图片](image/640-163918873097864.webp)理想状态下的窗口变化
...
![图片](image/640-163918873097865.webp)服务端繁忙状态下的窗口变化
...
![图片](image/640-163918873097866.webp)窗口大小在收缩

接着，发送方会**定时发送窗口大小探测报文**，以便及时知道接收方窗口大小的变化。

以下图 Wireshark 分析图作为例子说明：

![图片](image/640-163918873097867.webp)零窗口 与 窗口探测
...
![图片](image/640-163918873097868.webp)
...
![图片](image/640-163918873097869.webp)
...
![图片](image/640-163918873097870.webp)禁用 Nagle 算法 与 启用 Nagle 算法
...
![图片](data:image/gif;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVQImWNgYGBgAAAABQABh6FO1AAAAABJRU5ErkJggg==)关闭 Nagle 算法
...
![图片](image/640-163918873097871.webp)TCP 延迟确认

延迟等待的时间是在 Linux 内核中的定义的，如下图：

![图片](image/640-163918873097872.webp)

关键就需要 `HZ` 这个数值大小，HZ 是跟系统的时钟频率有关，每个操作系统都不一样，在我的 Linux 系统中 HZ 大小是 `1000`，如下图：

![图片](image/640-163918873097873.webp)
...
![图片](image/640-163918873097874.webp)关闭 TCP 延迟确认

> 延迟确认 和 Nagle 算法混合使用时，会产生新的问题

当 TCP 延迟确认 和 Nagle 算法混合使用时，会导致时耗增长，如下图：

![图片](image/640-163918873097875.webp)TCP 延迟确认 和 Nagle 算法混合使用
